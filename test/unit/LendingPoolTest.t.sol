...
contract LendingPoolTest is Test {
    ...
    function setUp() public {
        ...
        lending = new LendingPool(address(collateralToken), address(borrowToken));

        collateralToken.transfer(user, 10_000 ether);

        borrowToken.transfer(address(lending), 10_000 ether);
    }

    function testDepositCollateral() public {
        vm.startPrank(user);

        collateralToken.approve(address(lending), 1000 ether);

        lending.depositCollateral(1000 ether);

        vm.stopPrank();

        assertEq(lending.collateral(user), 1000 ether);
    }
    ...
    function testBorrow() public {
        ...
        assertEq(lending.debt(user), 500 ether);
    }
    ...
    function testRepay() public {
        ...
        borrowToken.approve(address(lending), 500 ether);
