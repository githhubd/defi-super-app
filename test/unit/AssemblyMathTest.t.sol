...
contract AssemblyMath {
    ...
    function multiplySolidity(uint256 a, uint256 b) external pure returns (uint256) {
        return a * b;
    }

    function multiplyAssembly(uint256 a, uint256 b) external pure returns (uint256 result) {
        assembly {
            result := mul(a, b)
        }
    }
